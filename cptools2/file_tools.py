...
import pandas as pd
import polars as pl
from cptools2.colours import pretty_print, yellow, purple, red, green


def merge_loaddata_metadata(chunk_output_csv, loaddata_csv, output_path=None):
    """
    Merge LoadData metadata into CellProfiler output CSV using ImageNumber as join key.
    
    This function joins metadata from a LoadData CSV (containing Metadata_* columns) into
    a CellProfiler output CSV (Image.csv, Cells.csv, etc.) using ImageNumber as the join key.
    The join happens at the chunk level BEFORE concatenation to preserve metadata integrity.
    
    Parameters:
    -----------
    chunk_output_csv : str
        Path to the CellProfiler output CSV file (e.g., raw_data/plate_0/Image.csv)
    loaddata_csv : str
        Path to the corresponding LoadData CSV file (e.g., loaddata/plate_0.csv)
    output_path : str, optional
        Path to write the enriched CSV. If None, overwrites chunk_output_csv in-place.
    ...
    dict : Dictionary containing:
        ...
        - 'null_metadata_count': int - Number of rows with null metadata (indicates ImageNumber mismatch)
    
    Raises:
    -------
    FileNotFoundError: If input files don't exist
    ValueError: If ImageNumber column not found in either file
    """
    result = {
        'success': False,
    ...
    try:
        ...
        if 'ImageNumber' not in output_df.columns:
            raise ValueError(f"ImageNumber column not found in output CSV: {chunk_output_csv}")
        if 'ImageNumber' not in loaddata_df.columns:
            raise ValueError(f"ImageNumber column not found in LoadData CSV: {loaddata_csv}")
        
        # Extract only Metadata_* columns from LoadData (plus ImageNumber for join key)
        metadata_cols = [col for col in loaddata_df.columns if col.startswith('Metadata_')]
        cols_to_keep = ['ImageNumber'] + metadata_cols
        
        # Select and validate columns exist
        available_cols = [col for col in cols_to_keep if col in loaddata_df.columns]
        if not available_cols:
            raise ValueError(f"No Metadata_* columns found in LoadData CSV: {loaddata_csv}")
        ...
        joined_df = output_df.join(loaddata_subset, on='ImageNumber', how='left')
        
        # Validate join integrity: count null metadata (indicates ImageNumber mismatch)
        if metadata_cols:
            # Count total nulls across all metadata columns
            null_count_df = joined_df.select(pl.col(metadata_cols).null_count())
            null_count = null_count_df.sum_horizontal()[0] if len(null_count_df) > 0 else 0
            result['null_metadata_count'] = null_count
            
            if null_count > 0:
                pretty_print(
                    f"Warning: {null_count} rows have null metadata after join - "
                    f"possible ImageNumber mismatch"
                )
...
def _discover_chunks_for_plate(raw_data_location, plate_name):
    ...
    Returns:
    --------
    list : Sorted list of chunk directory paths matching pattern plateName_*
    """
    chunks = []
    try:
...
def _discover_plates_from_raw_data(raw_data_location):
    ...
    try:
        ...
        pretty_print(f"Warning: Raw data location {raw_data_location} not found during plate discovery.")
        return []
    if not plate_names:
        pretty_print(f"Warning: No plate directories found in {raw_data_location}. Cannot join files.")
...
def enrich_chunks_with_metadata(location, patterns=None):
    ...
    This is the orchestration function that discovers all chunk/LoadData pairs and
    merges metadata into each chunk's output CSVs. This MUST happen before chunks
    are concatenated to preserve metadata integrity (ImageNumber is sequential per chunk).
    
    Parameters:
    -----------
    location : str
        Base location directory containing 'loaddata/' and 'raw_data/' subdirectories
    patterns : list, optional
        List of CSV filename patterns to enrich (e.g., ['Image.csv', 'Cells.csv'])
        If None, discovers all CSV files in chunks.
    ...
    if not os.path.exists(raw_data_location):
        pretty_print(f"Raw data directory not found: {red(raw_data_location)}")
        results['errors'].append(f"Raw data directory not found: {raw_data_location}")
        return results
    
    if not os.path.exists(loaddata_location):
        pretty_print(f"LoadData directory not found - skipping metadata enrichment")
        return results
    ...
    for plate_name in plate_names:
        ...
        for chunk_path in chunks:
            ...
            if not chunk_csvs:
                pretty_print(f"\t\tNo CSV files found in chunk")
                continue
            ...
            for csv_file in sorted(chunk_csvs):
                ...
                if merge_result['success']:
                    pretty_print(
                        ...
                        f"{purple(len(merge_result['metadata_columns_added']))} metadata columns"
                    )
                    results['total_files_enriched'] += 1
                    results['total_rows_enriched'] += merge_result['rows_after']
                    chunk_results.append(merge_result)
                else:
                    msg = f"\t\tFailed to enrich {csv_name}: {merge_result['error']}"
                    pretty_print(msg)
                    results['errors'].append(msg)
...
def join_plate_files(plate_store, raw_data_location, patterns=None, batch_id=None, enrich_metadata=True):
    """
    Join result files for each plate based on specified patterns.
    
    IMPORTANT: Metadata enrichment happens BEFORE concatenation. This is critical because
    ImageNumber is sequential within each chunk (1-96, 1-96, ...) and resets per chunk.
    If we concatenate first, ImageNumber loses meaning. By enriching each chunk individually
    before concatenation, we preserve metadata integrity.
    ...
    if enrich_metadata:
        ...
        if enrich_results['errors']:
            pretty_print(f"Enrichment completed with {len(enrich_results['errors'])} warnings/errors")
        else:
            pretty_print(f"Enrichment successful!")
